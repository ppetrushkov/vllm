...
from vllm.lora.punica_wrapper.utils import compute_meta, convert_mapping
...
class RoadWrapperABC(ABC):
    ...
    def add_linear(self,
                   x: torch.Tensor,
                   theta_stacked: tuple[torch.Tensor, ...],
                   alpha_stacked: tuple[torch.Tensor, ...],
...
class RoadWrapperBase(RoadWrapperABC):
    """
    ...
    def update_metadata(
        ...
        self._update_base_metadata(mapping, adapter_index_to_id, max_loras,
...
def get_road_wrapper(*args, **kwargs) -> RoadWrapperBase:
    road_wrapper_qualname = current_platform.get_road_wrapper()
    road_wrapper_cls = resolve_obj_by_qualname(road_wrapper_qualname)
    road_wrapper = road_wrapper_cls(*args, **kwargs)
    assert road_wrapper is not None, \
        "the punica_wrapper_qualname(" + road_wrapper_qualname + ") is wrong."
    logger.info_once("Using %s.", road_wrapper_qualname.rsplit(".", 1)[1])
    return road_wrapper
